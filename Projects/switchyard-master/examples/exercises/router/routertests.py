...
def case_10(s):
    ...
    for i in range(5):
        ...
        if i == 3:
            testPkt5 = mk_pkt(EtherType.IPv4, '30:00:00:00:10:04', '100.1.240.56', '100.1.230.57', '99:99:99:99:99:99')
            s.expect(PacketInputEvent("router-eth0", testPkt5, display=Ethernet), "IP packet to'100.1.230.57' should arrive on router-eth0")
        s.expect(PacketInputTimeoutEvent(1.0), "Wait for 1.0 second")

    # send host unreachable error message back to '100.1.230.56'
    ...
    # send host unreachable error message back to '100.1.240.56'
    arpReqPkt = mk_pkt(EtherType.ARP, '10:00:00:00:00:03', '100.1.220.2', '100.1.240.56')
    s.expect(PacketOutputEvent("router-eth2", arpReqPkt, display=Ethernet),
             "ARP request for '100.1.240.56' from router-eth2")

    arpRespPkt = mk_pkt(EtherType.ARP, '30:00:00:00:10:04', '100.1.240.56', '100.1.220.2', '10:00:00:00:00:03', True)
    s.expect(PacketInputEvent("router-eth2", arpRespPkt, display=Ethernet), "ARP respond to router-eth2")


    temp2 = mk_pkt(EtherType.IPv4, '30:00:00:00:10:04', '100.1.240.56', '100.1.230.57', '99:99:99:99:99:99')
    temp2.get_header(IPv4).ttl = 31
    errMsgPkt5 = mk_icmp_error_pkg(temp2, ICMPType.DestinationUnreachable,
                                   ICMPTypeCodeMap[ICMPType.DestinationUnreachable].HostUnreachable, '10:00:00:00:00:03',
                                   '30:00:00:00:10:04', '100.1.220.2')
    errMsgPkt5.get_header(IPv4).ttl -= 1
    s.expect(PacketOutputEvent("router-eth2", errMsgPkt5, display=ICMP), "ICMP error reply for ARP failure via router-eth2")
